...
# ── 启动ChatTTS服务 ──
echo "[3/3] 启动视频合成服务 (端口 5006)..."
if lsof -i :5006 &>/dev/null; then
    echo -e "  ${YELLOW}端口 5006 已被占用，跳过${NC}"
else
    nohup python3 -m services.chattts.app \
        > logs/chattts_service.log 2>&1 &
    PID2=$!
    echo "$PID2" > logs/chattts_service.pid
    echo -e "  ${GREEN}ChatTTS服务已启动 (PID: $PID2)${NC}"
    echo "  日志: logs/chattts_service.log"
fi
...
if curl -s http://localhost:5006/health | grep -q "healthy"; then
    echo -e "  ${GREEN}✓ ChatTTS服务 (5006)${NC}"
else
    echo -e "  ${YELLOW}! ChatTTS服务启动中...请稍后检查${NC}"
fi
...
echo "    ChatTTS服务: http://localhost:5006/docs"
echo ""
